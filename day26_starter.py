...
def pattern_zero_l1_hits(n=20):
    """
    To get 0% hits, we access L1_SIZE + 1 unique addresses.
    Example: 0, 1, 2, 3, 4 ... then back to 0.
    By the time we ask for 0 again, the access of '4' evicted it.
    """
    cycle = list(range(L1_SIZE + 1)) 
...
def pattern_perfect_l1(n=20):
    """
    To get near 100% hits, we only access addresses that fit in L1.
    After the first 4 accesses (cold misses), every subsequent access is a hit.
    """
    cycle = list(range(L1_SIZE)) 
    return [cycle[i % len(cycle)] for i in range(n)]
...
PATTERNS = {
    "1": ("Sequential (spatial locality)",  pattern_sequential()),
    "2": ("Repeated (temporal locality)",   pattern_repeated()),
    "3": ("Strided (cache-unfriendly)",     pattern_strided()),
    "4": ("Random (worst case)",            pattern_random()),
    "5": ("Zero L1 hits (TODO #1)",         pattern_zero_l1_hits()),
    "6": ("Perfect L1 hits (TODO #2)",      pattern_perfect_l1()),
}
